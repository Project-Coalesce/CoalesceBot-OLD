...
data class Punishment(val reason: Reason, val punished: User, val punishee: User, val description: String?, var expiration: Long?) {
    fun doActUpon(record: Collection<Punishment>, channel: MessageChannel) {
        val amount = record.size + 1 // Past record + this
        val totalSev = run {
            var ret = 0
            for ((punishment) in record) {
                ret += punishment.severity
                if (ret >= 7) {
                    ret = 7
                    break
                }
        ...
        fun message(channel: MessageChannel, embed: EmbedBuilder, warning: Boolean, until: String?) {
            ...
            val logChannel = Bot.instance.jda.getTextChannelById(299385639437074433) // TODO: Make channel configurable per guild.
